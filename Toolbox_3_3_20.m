...
% Motor Information
%{
structure of "x_motor_data" matrices:
motor_data(:,1): power consumption, W
motor_data(:,2): thrust generated, N
motor_data(:,3): current required, A
%}
% VTOL
motor_data = motor_find(motor_input_v);
vtol_motor_data = [0 0 0; motor_data(:,3) g*0.001*motor_data(:,2) motor_data(:,1)];
% Horizontal
motor_data = motor_find(motor_input_h);
horz_motor_data = [0 0 0; motor_data(:,3) g*0.001*motor_data(:,2) motor_data(:,1)];
battery_data = battery_find(battery_input);

% Account for possible design issues if ESC has a lower power limit than
% the motor. This code assumes all motors have the same ESC
Motor_max_current_h = horz_motor_data(end,3);
Motor_max_current_v = vtol_motor_data(end,3);

if Motor_max_current_h > ESC_max_current
    % If ESC rather than motor itself limits circuit, cutoff data at max
    % ESC current
    Max_data_h = interp1(horz_motor_data(:,3),horz_motor_data,ESC_max_current);
    ind = find(horz_motor_data(:,3)<ESC_max_current,1,'last');
    horz_motor_data = [horz_motor_data(1:ind,:); Max_data_h];
    Tmax = horz_motor_data(end,2);
    fprintf('Warning: ESC undersized for horizontal motor!\n')
else
    Tmax = horz_motor_data(end,2)
end

if Motor_max_current_v > ESC_max_current
    % If ESC rather than motor itself limits circuit, cutoff data at max
    % ESC current
    Max_data_v = interp1(vtol_motor_data(:,3),vtol_motor_data,ESC_max_current);
    ind = find(vtol_motor_data(:,3)<ESC_max_current,1,'last');
    vtol_motor_data = [vtol_motor_data(1:ind,:); Max_data_v];
    fprintf('Warning: ESC undersized for vtol motors!\n')
end
...
fprintf('The Power needed for horizontal flight is %f W\n',P_horz')

%% Turning Calculations

CL_max = air_foil_data(3);
R_min = Radius_of_turn(V,W_L,S,Tmax,Cd0,CL_max,e,AR,K);
fprintf('Minimum radius of turn: %f meters\n',R_min);
...
if input('Test combined transition? 1 = Y, 0 = N: ') == 1
    x0_combo = [-6 0 -V];
